describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const prodItems = await page.$$('product-item');
    const shadowRoot = await (await prodItems[0].getProperty("shadowRoot")).asElement();
    const btn = await shadowRoot.$('button');
    await btn.click();
    let innrTxt = await btn.getProperty('innerText');
    let btnText = innrTxt['_remoteObject'].value;
    //let btnText = await page.evaluate(el => el.textContent, btn);
    // Click again to empty cart
    await btn.click();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const prodItems = await page.$$('product-item');
    // Click all 20 buttons
    for (let i = 0; i < 20; i++) {
      const shadowRoot = await (await prodItems[i].getProperty("shadowRoot")).asElement();
      const btn = await shadowRoot.$('button');
      await btn.click();
    }
    // Check cart count
    const cartCount = await page.$('span#cart-count');
    let innrTxt = await cartCount.getProperty('innerText');
    let cartCnt = innrTxt['_remoteObject'].value;
    expect(cartCnt).toBe('20');
  }, 10000);

  // Check to make sure that after you reload the page it remembers all of the items in your cart
  it('Checking number of items in cart on screen after reload', async () => {
    console.log('Checking number of items in cart on screen after reload...');
    let allButtonsRemove = true;
